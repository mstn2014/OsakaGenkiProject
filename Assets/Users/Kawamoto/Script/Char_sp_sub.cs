...
public class Char_sp_sub : MonoBehaviour {
	...
	public int SetFig{
		set{
			if(value == 1){
				GoFlag = true;
				SerectFlag = true;
				iTween.MoveTo(this.gameObject, iTween.Hash( "position", GameObject.Find("Spot_R").transform.position, "time",D_time ,"easetype",iTween.EaseType.linear) );
			}
