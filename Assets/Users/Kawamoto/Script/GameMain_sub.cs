...
public class GameMain_sub : MonoBehaviour {
	...
	public void CharMoveOrder(){
		...
		m_obj [ren] = null;
		NewModelMake ();
		m_Char_sp = obj.GetComponent<Char_sp_sub> ();
		m_Char_sp.SetFig = 1;
	}
	...
	public void NewModelMake(){
		for(int i = 0;i<MaxChar;i++)
		{
			if( m_obj[i] == null)
			{
				m_obj[i] = CreatePrefab.InstantiateGameObject(m_char,pos[i],Quaternion.identity,sce[i],GameObject.Find("CharMane_sub"));
	...
	public void ObjInList(GameObject obj){
		obj.GetComponent<Char_sp_sub> ().enabled = false;	// 移動スクリプトを無効化
